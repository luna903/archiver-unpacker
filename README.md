...
Использует **исключительно стандартную библиотеку Python** (3.12+).
...
| 3.12 / 3.13   | `zlib`-резерв (совместимый формат ZLBK)     | ✅ встроенный  |
...
# Версия и используемый zstd-бэкенд
...
|---------------------|------------------------------------------|
| `compression.zstd`  | Python 3.14+ (нативный, предпочтительный)|
| `zstandard`         | Пакет `zstandard` установлен             |
| `zlib-fallback`     | Резервный вариант через zlib             |

> ⚠️ Архивы в `zlib-fallback` режиме совместимы только с этой утилитой.  
